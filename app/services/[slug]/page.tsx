...
export default async function ServiceDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const avg = rated.length > 0 ? rated.reduce((a, r) => a + (r.rating ?? 0), 0) / rated.length : 0;

  return (
    <div className="container-page py-10">
      <Link href="/services" className="text-sm text-muted hover:text-ink">← Marketplace</Link>
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {rated.length > 0 && (
              <span className="badge !text-ink"><Stars value={avg} /> {avg.toFixed(1)} ({rated.length})</span>
            )}
          </div>
          ...
          <div className="card mt-6 p-5">
            ...
            <code className="mt-1 block break-all font-mono text-sm text-ink">{service.method} {service.endpoint}</code>
          </div>

          <div className="card mt-4 p-5">
            <p className="text-xs uppercase tracking-wide text-muted">Sample response</p>
            <pre className="mt-2 overflow-x-auto rounded-lg bg-bg p-4 font-mono text-xs text-mint">{JSON.stringify(service.sampleResponse, null, 2)}</pre>
          </div>
        </div>
        ...
        <aside className="space-y-4">
          <div className="card p-6">
            ...
            <p className="mt-1 text-4xl font-extrabold text-mint">{usd(service.price)}</p>
            <p className="mt-1 text-xs text-muted">USDC · settled on Arc via x402</p>
            <TryService endpoint={service.endpoint} method={service.method} price={service.price} />
            <Link href="/playground" className="btn-ghost mt-3 w-full">Buy with the agent →</Link>
          </div>
