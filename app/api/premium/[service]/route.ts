...
import { recordPayment, recordReceipt } from "@/lib/store";
...
/** Produce the actual paid payload for a service. */
async function produce(slug: string, fallback: unknown): Promise<unknown> {
  if (slug === "market-insight") {
    // Flagship seller: AuraPredict. Try the live indexer, fall back to sample.
...
async function handle(req: NextRequest, slug: string) {
  ...
  if (!payment) {
    // 402 Payment Required — advertise how to pay.
    return NextResponse.json(buildChallenge(service.price, service.sellerAddress), {
      status: 402,
    });
  }
  ...
  // Record payment + a quality receipt (proof of paid + result hash).
