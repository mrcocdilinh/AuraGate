    ...
    <div className="container-page py-10">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{w.status === "connected" ? `Signed in as ${shortAddr(w.address)}` : "Connect a wallet to receive USDC"}</p>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {receipts.length === 0 && (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-muted">No payments yet — run the agent or try a service.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function RegisterService({ address, onCreated }: { address?: string; onCreated: () => void }) {
  const [form, setForm] = useState({ name: "", description: "", price: "0.01", category: "data" });
  ...
  async function submit() {
    ...
    const res = await fetch("/api/services", {
      ...
      body: JSON.stringify({ ...form, sellerAddress: address ?? "0x0000000000000000000000000000000000000000", sellerName: "You" }),
    });
    if (res.ok) {
  ...
  return (
    <div className="card p-5">
      ...
      <p className="mt-1 text-xs text-muted">Register an API and start accepting USDC per request.</p>
      <div className="mt-4 space-y-3">
        <input className="input" placeholder="Service name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        <textarea className="input min-h-20" placeholder="What does it do?" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
        <div className="flex gap-3">
          <input className="input" placeholder="Price USDC" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} />
          <select className="input" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
            {["data", "ai", "oracle", "compute", "market-insight"].map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <button className="btn-primary w-full" onClick={submit} disabled={busy || !form.name}>{busy ? "Listing…" : "List service"}</button>
